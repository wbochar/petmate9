...
// the lower-case font.  We append a single addon glyph (X) used by the
// picker's VDC transparent block row.
export const c128DataVdc: number[] = (() => {
  ...
  const barMinimal = fs.readFileSync(path.resolve(appPath, 'assets/bar-minimal.bin'))
  const transparentGlyph = Buffer.from(barMinimal).slice(0, 8)
  return Array.from(new Uint8Array(Buffer.concat([Buffer.from(upper), Buffer.from(lower), transparentGlyph])))
})()
